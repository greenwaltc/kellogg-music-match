...
class ApiClient {
  ...
  Future<Map<String, dynamic>> postJson(String path, Map<String, dynamic> body, {String? bearerToken}) async {
    final uri = Uri.parse('$baseUrl$path');
    final headers = {..._jsonHeaders};
    if (bearerToken != null && bearerToken.isNotEmpty) {
      headers['Authorization'] = 'Bearer $bearerToken';
    }
    final resp = await _http.post(uri, headers: headers, body: jsonEncode(body));
    ...
    String extractMessage(String body) {
      try {
        ...
        if (decoded is Map<String, dynamic>) {
          ...
          if (errors is List) {
            final parts = errors.whereType<String>().map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
  ...
  final msg = extractMessage(resp.body);
    ...
    throw ApiException(resp.statusCode, msg.isNotEmpty ? msg : 'Request failed (${resp.statusCode})', details: details);
  }
}
...
String get _defaultBaseUrl {
  ...
  // Prefer ngrok tunnel for easier device/emulator testing; override with --dart-define if needed.
  const ngrok = 'https://twolegged-leonida-interchangeably.ngrok-free.dev';
  if (ngrok.isNotEmpty) return ngrok;
