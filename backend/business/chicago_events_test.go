...
func TestConcertService_GetChicagoEvents(t *testing.T) {
	...
	t.Run("GetChicagoEventsSuccess", func(t *testing.T) {
		events, count, err := service.GetChicagoEvents(ctx, nil, false, 10, 0, false, nil, nil)
	...
	t.Run("GetChicagoEventsWithArtistFilter", func(t *testing.T) {
		artistName := "Test"
		events, count, err := service.GetChicagoEvents(ctx, &artistName, false, 10, 0, false, nil, nil)
		require.NoError(t, err)
	...
	t.Run("GetChicagoEventsNoRepository", func(t *testing.T) {
		...
		_, _, err := serviceWithoutRepo.GetChicagoEvents(ctx, nil, false, 10, 0, false, nil, nil)
